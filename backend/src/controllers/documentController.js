...
const normalizeSubmittedRequestStatus = (value) => {
  const normalizedValue = String(value || '').trim().toLowerCase();
  const allowedValues = new Set(['approved', 'disapproved', 'pending', 'submitted']);
  return allowedValues.has(normalizedValue) ? normalizedValue : 'pending';
};

const normalizeSubmittedRequestSelectionStatus = (value) => {
  const normalizedValue = String(value || '').trim().toLowerCase();
  const allowedValues = new Set(['approved', 'disapproved', 'pending']);
  return allowedValues.has(normalizedValue) ? normalizedValue : 'pending';
};

const sanitizeSubmittedRequests = (rawValue) => {
  ...
  for (const [serviceKey, status] of entries) {
    const normalizedServiceKey = String(serviceKey || '').trim().toLowerCase();
    if (!normalizedServiceKey) continue;
    normalized[normalizedServiceKey] = normalizeSubmittedRequestStatus(status);
  }

  return normalized;
};

const sanitizeSubmittedRequestSelections = (rawValue) => {
  if (!rawValue || typeof rawValue !== 'object' || Array.isArray(rawValue)) {
    return {};
  }
  ...
  for (const [serviceKey, status] of entries) {
    const normalizedServiceKey = String(serviceKey || '').trim().toLowerCase();
    if (!normalizedServiceKey) continue;
    normalized[normalizedServiceKey] = normalizeSubmittedRequestSelectionStatus(status);
  }
...
// @desc    Update submitted request selections for a grouped submission
// @route   PUT /api/documents/:id/submitted-requests
// @access  Private (Non-admin)
exports.updateSubmittedRequests = async (req, res) => {
  try {
    if (isAdminUser(req.user)) {
      return res.status(403).json({
        status: 'error',
        message: 'Admin users can only view submitted request selections',
      });
    }
    ...
    if (!hasAccess) {
      return res.status(403).json({
        status: 'error',
        message: 'You can only update submitted requests for your own submission',
      });
    }

    const incomingSubmittedRequests = sanitizeSubmittedRequests(req.body?.submittedRequests);
    if (!Object.keys(incomingSubmittedRequests).length) {
      return res.status(400).json({
        status: 'error',
        message: 'submittedRequests is required',
      });
    }

    const submissionQuery = getSubmissionQuery(document);
    const docsToUpdate = await Document.find(submissionQuery).sort({ createdAt: -1 });
    const targets = docsToUpdate.length ? docsToUpdate : [document];

    for (const docItem of targets) {
      const existingMetadata = docItem.metadata && typeof docItem.metadata === 'object'
        ? { ...docItem.metadata }
        : {};

      const existingSubmittedRequests = sanitizeSubmittedRequests(existingMetadata.submittedRequests);
      const existingSubmittedRequestSelections = sanitizeSubmittedRequestSelections(existingMetadata.submittedRequestSelections);

      const mergedSubmittedRequests = {
        ...existingSubmittedRequests,
      };

      const mergedSubmittedRequestSelections = {
        ...existingSubmittedRequestSelections,
      };

      for (const [serviceKey, status] of Object.entries(incomingSubmittedRequests)) {
        if (status === 'submitted') {
          mergedSubmittedRequests[serviceKey] = 'submitted';
          if (!mergedSubmittedRequestSelections[serviceKey]) {
            mergedSubmittedRequestSelections[serviceKey] = 'pending';
          }
          continue;
        }

        mergedSubmittedRequests[serviceKey] = status;
        mergedSubmittedRequestSelections[serviceKey] = normalizeSubmittedRequestSelectionStatus(status);
      }

      docItem.metadata = {
        ...existingMetadata,
        submittedRequests: mergedSubmittedRequests,
        submittedRequestSelections: mergedSubmittedRequestSelections,
        submittedRequestsUpdatedAt: new Date().toISOString(),
        submittedRequestsUpdatedBy: String(req.user._id),
      };

      await docItem.save();
    }

    const responseDocument = targets[0];

    return res.status(200).json({
      status: 'success',
      message: 'Submitted request selections updated successfully',
      data: {
        document: responseDocument,
        submittedRequests: responseDocument.metadata?.submittedRequests || {},
        submittedRequestSelections: responseDocument.metadata?.submittedRequestSelections || {},
      },
    });
  } catch (error) {
    console.error('Update submitted requests error:', error);
    return res.status(500).json({
      status: 'error',
      message: 'Error updating submitted request selections',
    });
  }
};
