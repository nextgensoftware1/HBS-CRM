...
export const documentService = {
	...
	async updateSubmittedRequests(
		id: string,
		submittedRequests: Record<string, 'approved' | 'disapproved' | 'pending' | 'submitted'>,
	): Promise<{
		submittedRequests: Record<string, string>;
		submittedRequestSelections: Record<string, string>;
	}> {
		const response = await api.put(`/documents/${id}/submitted-requests`, {
			submittedRequests,
		});

		return {
			submittedRequests: response.data?.data?.submittedRequests || {},
			submittedRequestSelections: response.data?.data?.submittedRequestSelections || {},
		};
	},
